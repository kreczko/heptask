...
.. code-block:: python
    ...
    import luigi
    import os
    from heptask.batch.htcondor import HTCondorJobTask
...
def _build_job_description(job_params):
    ...
    submit_params = {k: str(v) for k, v in submit_params.items()}
...
class HTCondorJobTask(luigi.Task):
    ...
    def _run_job(self):
        ...
        self.jobLog = os.path.join(self.tmp_dir, 'job.log')
        job_params = {
            'Executable': run_sh,
        ...
        jobAD = htcondor.Submit(job_desc)
        # Create a schedd object using default settings.
        schedd = htcondor.Schedd()
        with schedd.transaction() as txn:
            self.job_id = jobAD.queue(txn)
        # self.job_id = _parse_condor_submit_job_id(output)
        logger.debug(
            "Submitted job to htcondor with response: jobID:" + str(self.job_id))
    ...
    def _track_job(self):
        ...
        > transferring output
        """
        # use log file to track job until it ends
